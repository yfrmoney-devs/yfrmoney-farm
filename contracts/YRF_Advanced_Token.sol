...
contract YRF_Token is Context, IBEP20, Ownable {
	...
	function saleLockedTokens(
		address[] memory holders,
		uint256[] memory tokens,
		uint256[] memory lockedTokens,
		uint256 lockTimeInDays
	) public onlyOwner returns (bool) {
		uint256 releaseTime = block.timestamp + (lockTimeInDays * 86400);

		for (uint256 i = 0; i < holders.length; i++) {
			address recipient = holders[i];
			_tokenLockedList[recipient] = TokenLock({
				holder: recipient,
				lockedAmount: lockedTokens[i],
	...
	function _transfer(
		...
		if (
			...
			require(
				remainingTokens >= _tokenLockedList[sender].lockedAmount,
				"BEP20: your tokens is locked. Please to wait until its unlocked"
			);
		}
