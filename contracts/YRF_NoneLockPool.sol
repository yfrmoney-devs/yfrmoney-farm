 ...
 * the owner.
 */
contract YRF_NoneLockPool is Ownable {
	using SafeMath for uint256;
	...
	uint256 public _transactionFeeInWei = 0; // 0.0 BNB
	...
	modifier pathUpdateReward(uint256 timestamp) {
		uint256 currentValue =
			timestamp > _poolEndedAt ? _poolEndedAt : timestamp;
		...
		returns (bool success_, uint256 stakeAmount_)
	{
		...
		uint256 addToTotalAmount =
			addOrUpdateStakeholder(stakeholder, _stakeAmount, timestamp);
		...
		returns (bool success_, uint256 amount_)
	{
		address stakeholder = msg.sender;
		uint256 timestamp = block.timestamp;
		(bool claimSuccess_, uint256 claimedAmount_) =
			claimRewardInternal(stakeholder, timestamp);
		...
		)
	{
		...
		uint256 poolSpentTimeInSeconds =
			lastTimePeriod.sub(_userStakes[_stakeholder].lastRewardUpdatedAt);
		...
		returns (uint256)
	{
		...
		uint256 poolSpentSeconds =
			lastTimePeriod.sub(_poolRewardDistributionStartedAt);
		...
		)
	{
		...
		uint256 _poolDistributedReward =
			getPoolDistributedReward(block.timestamp);
		uint256 _poolRemainingReward =
			_poolTotalReward.sub(_poolDistributedReward);
